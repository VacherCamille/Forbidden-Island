...
public class PlateauJeu extends Observe {
    ...
    private void initGrille() {
        for (int i = 0; i < 6; i++) {
            for (int j = 0; j < 6; j++) {
                //JButton buttonTuile = this.createButton(grille.getTuile(i, j));
                //panelGrille.add(buttonTuile);
                TuileGraphique tuile = new TuileGraphique(grille.getTuile(i, j));
                panelGrille.add(tuile);
            }
        }
    }
    
    /*private JButton createButton(Tuile tuile) {
        JButton button;
        if (tuile != null) {
            button = new JButton(tuile.getNomTuile());
            if (tuile.getSpawnPion() != null) {
                button.setBackground(tuile.getSpawnPion().getCouleur());
            }
        } else {
